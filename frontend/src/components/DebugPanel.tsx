      ...
      <div className={`${isDark ? 'border-[#3F3F46] bg-[#242426]' : 'border-[#D1D5DB] bg-white'} border overflow-hidden flex flex-col h-full ${className}`}>
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center">
            ...
            <p className={`mt-2 ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>Running debug analysis...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (!debugData && !validationResult) {
    return (
      <div className={`${isDark ? 'border-[#3F3F46] bg-[#242426]' : 'border-[#D1D5DB] bg-white'} border overflow-hidden flex flex-col h-full ${className}`}>
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center">
            <Bug size={48} className={`mx-auto mb-4 ${isDark ? 'text-[#3F3F46]' : 'text-[#D1D5DB]'}`} />
            <p className={isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}>Debug information will appear here</p>
            <p className={`text-sm mt-2 ${isDark ? 'text-[#6B7280]' : 'text-[#9CA3AF]'}`}>
  ...
  return (
    <div className={`${isDark ? 'border-[#3F3F46] bg-[#242426]' : 'border-[#D1D5DB] bg-white'} border overflow-hidden flex flex-col h-full ${className}`}>
      ...
      <div className={`shrink-0 ${isDark ? 'bg-[#2D2E30] border-[#3F3F46]' : 'bg-[#F9FAFB] border-[#D1D5DB]'} border-b p-4 flex items-center gap-2`}>
        <Bug size={18} className={isDark ? 'text-[#60A5FA]' : 'text-[#2563EB]'} />
        <h3 className={`font-semibold ${isDark ? 'text-[#F3F4F6]' : 'text-[#111827]'}`}>Debug Panel</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className={`border-b ${isDark ? 'border-[#3F3F46]' : 'border-[#D1D5DB]'}`}>
            ...
            >
              <div className="flex items-center gap-3">
                {validationResult.valid ? (
                  <CheckCircle size={16} className={isDark ? 'text-[#10B981]' : 'text-[#059669]'} />
                ) : (
                  <XCircle size={16} className={isDark ? 'text-[#F87171]' : 'text-[#DC2626]'} />
                )}
            ...
            {expandedSections.has('validation') && (
              <div className={`p-4 ${isDark ? 'bg-[#1E1E1E]' : 'bg-[#F9FAFB]'}`}>
                {validationResult.valid ? (
                  <div className={`flex items-center gap-2 ${isDark ? 'text-[#10B981]' : 'text-[#059669]'}`}>
                    <CheckCircle size={20} />
                    <span>Code is syntactically valid</span>
                  </div>
                ) : (
          ...
          <div className={`border-b ${isDark ? 'border-[#3F3F46]' : 'border-[#D1D5DB]'}`}>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>
            ...
            {expandedSections.has('tokens') && (
              <div className={`p-4 ${isDark ? 'bg-[#1E1E1E]' : 'bg-[#F9FAFB]'}`}>
                <div className={`overflow-x-auto ${isDark ? 'bg-[#242426] border-[#3F3F46]' : 'bg-white border-[#D1D5DB]'} border rounded`}>
                  <table className="w-full text-sm">
                    <thead className={isDark ? 'bg-[#2D2E30]' : 'bg-[#F3F4F6]'}>
                      <tr>
                        <th className={`px-4 py-2 text-left ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>#</th>
                        <th className={`px-4 py-2 text-left ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>Type</th>
                        <th className={`px-4 py-2 text-left ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>Value</th>
                        <th className={`px-4 py-2 text-left ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>Line</th>
                      </tr>
                    </thead>
                    <tbody>
                      {debugData.tokens.tokens.map((token: any, index: number) => (
                        <tr 
                          key={index}
                          className={`border-t ${isDark ? 'border-[#3F3F46] hover:bg-[#2D2E30]' : 'border-[#D1D5DB] hover:bg-[#F9FAFB]'}`}
                        >
                          <td className={`px-4 py-2 ${isDark ? 'text-[#6B7280]' : 'text-[#9CA3AF]'}`}>
                            {index + 1}
                          </td>
                          <td className={`px-4 py-2 font-mono ${isDark ? 'text-[#60A5FA]' : 'text-[#2563EB]'}`}>
                            {token.type}
                          </td>
                          <td className={`px-4 py-2 font-mono ${isDark ? 'text-[#F3F4F6]' : 'text-[#111827]'}`}>
                            {token.value}
                          </td>
                          <td className={`px-4 py-2 ${isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}`}>
                            {token.line}
                          </td>
                        </tr>
                      ))}
        ...
        {debugData?.parsing && (
          <div className={`border-b ${isDark ? 'border-[#3F3F46]' : 'border-[#D1D5DB]'}`}>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {debugData.parsing.success ? (
                  <CheckCircle size={14} className={isDark ? 'text-[#10B981]' : 'text-[#059669]'} />
                ) : (
                  <XCircle size={14} className={isDark ? 'text-[#F87171]' : 'text-[#DC2626]'} />
                )}
              </div>
              <span className={isDark ? 'text-[#A1A1AA]' : 'text-[#6B7280]'}>
            ...
            {expandedSections.has('parsing') && (
              <div className={`p-4 ${isDark ? 'bg-[#1E1E1E]' : 'bg-[#F9FAFB]'}`}>
                {debugData.parsing.success ? (
                  <pre className={`text-sm ${isDark ? 'bg-[#242426] text-[#F3F4F6] border-[#3F3F46]' : 'bg-white text-[#111827] border-[#D1D5DB]'} p-3 rounded border overflow-x-auto max-h-60`}>
                    {debugData.parsing.ast}
                  </pre>
                ) : (
                  <div className="space-y-2">
                    {debugData.parsing.errors?.map((error: string, index: number) => (
                      <div 
                        key={index}
                        className={`flex items-start gap-2 p-3 rounded border ${
                          isDark 
        ...
        {debugData?.semantic && (
          <div>
            ...
            >
              <div className="flex items-center gap-3">
                <Code2 size={16} className={isDark ? 'text-[#F97316]' : 'text-[#D97706]'} />
                <span className={`font-semibold ${isDark ? 'text-[#F3F4F6]' : 'text-[#111827]'}`}>
                  Semantic Analysis
                </span>
            ...
            {expandedSections.has('semantic') && (
              <div className={`p-4 ${isDark ? 'bg-[#1E1E1E]' : 'bg-[#F9FAFB]'} space-y-4`}>
                ...
                {/* Errors */}
                {debugData.semantic.errors && debugData.semantic.errors.length > 0 && (
                  <div>
                    <h4 className={`font-semibold mb-2 ${isDark ? 'text-[#F87171]' : 'text-[#DC2626]'}`}>
                      Errors ({debugData.semantic.errors.length})
                    </h4>
                    <div className="space-y-2">
                      {debugData.semantic.errors.map((error: any, index: number) => (
                        <div 
                          key={index}
                          className={`flex items-start gap-2 p-3 rounded border ${
                            isDark 
                ...
                {/* Warnings */}
                {debugData.semantic.warnings && debugData.semantic.warnings.length > 0 && (
                  <div>
                    <h4 className={`font-semibold mb-2 ${isDark ? 'text-[#FBBF24]' : 'text-[#D97706]'}`}>
                      Warnings ({debugData.semantic.warnings.length})
                    </h4>
                    <div className="space-y-2">
                      {debugData.semantic.warnings.map((warning: any, index: number) => (
                        <div 
                          key={index}
                          className={`flex items-start gap-2 p-3 rounded border ${
                            isDark 
                        ...
                        >
                          <AlertTriangle size={16} className="shrink-0 mt-0.5" />
                          <div className="text-sm">
                            {warning.line && <span className="font-semibold">Line {warning.line}: </span>}
                            {warning.message}
                          </div>
                        </div>
                      ))}
