...
export default function CompilerPage() {
  ...
  const [compilationResult, setCompilationResult] =
    useState<CompilationResponse | null>(null);
  const [securityAnalysis, setSecurityAnalysis] =
    useState<SecurityAnalysisResponse | null>(null);
  ...
  const compiledPolicy = useMemo(() => {
    if (
      compilationResult?.stage === "semantic_analysis" &&
    ...
    if (
      compilationResult?.success &&
      compilationResult.stages?.code_generation?.generated_code
    ) {
      try {
        return JSON.parse(
          compilationResult.stages.code_generation.generated_code
  ...
  const errors = useMemo(() => {
    ...
    // Check if compilation was blocked at semantic analysis stage
    if (
      !compilationResult.success &&
      compilationResult.stage === "semantic_analysis"
    ) {
      // Add a prominent blocking message at the top
      allErrors.push({
        line: 0,
        message:
    ...
    if (
      !compilationResult.success &&
      compilationResult.stages?.parsing?.errors
    ) {
    ...
    if (compilationResult.stages?.semantic_analysis) {
      ...
      compilationResult.stages.semantic_analysis.warnings?.forEach(
        (warning) => {
          allErrors.push({
            line: warning.line || 1,
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab === "compilation" ? (
          ...
          <RiskReport securityAnalysis={securityAnalysis} className="h-full" />
        ) : (
          <DebugPanel
            debugData={debugData}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            <button
              ...
              title={
                isResultsVisible ? "Hide results panel" : "Show results panel"
              }
            >
